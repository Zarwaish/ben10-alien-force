...
async function main() {
  console.log('Starting scan of all poolers (indices 0 to 3, all regions) WITH SNI servername...');
  ...
  for (const index of [0, 1, 2, 3]) {
    for (const region of regions) {
      ...
      const client = new Client({
        ...
        connectionTimeoutMillis: 1500 // 1.5 seconds timeout for fast scanning
      });
  ...
  console.log('\nScan completed. No pooler matched with SNI.');
}
